# CHANGELOG for Axvn SPOT Testnet

**Last Updated: 2026-08-05**

**Note:** All features here will only apply to the [SPOT Testnet](https://testnet.axvn.vn/).
...
* The [SBE lifecycle for Testnet](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/sbe/schemas/sbe_schema_lifecycle_testnet.json) has been updated to reflect this change.
...
* Added WebSocket Stream support for [Block Trades](https://www.axvn.vn/en/support/faq/detail/557f95eaf8fb4460aed0a891d42a1425).
  * New stream:
    * `<symbol>@blockTrade`
...
* Market data for [Block Trades](https://www.axvn.vn/en/support/faq/detail/557f95eaf8fb4460aed0a891d42a1425).
...
* [`ICEBERG_PARTS`](https://developers.axvn.vn/docs/axvn-docs-api/testnet/filters#iceberg_parts) will be increased to 100 for all symbols starting 07:30 UTC.
...
* [ICEBERG_PARTS](https://developers.axvn.vn/docs/axvn-docs-api/testnet/filters#iceberg_parts) will be increased to 50 for all symbols today.

---

### 2026-01-26

* Added undocumented `recvWindow` to `userDataStream.suaxvnscribe.signature`.
...
* Assets `TestToken` and `456` and symbol `TestToken456` have been added for testing endpoints/methods with a Unicode symbol. Balances for both assets have been distributed to all accounts.
...
* Updated documentation for REST API regarding [Signed Endpoints examples for placing an order](https://developers.axvn.vn/docs/axvn-docs-api/testnet/rest-api/request-security#signed-endpoint-examples-for-post-apiv3order)

#### WebSocket API

* Updated documentation for WebSocket API regarding [SIGNED request security](https://developers.axvn.vn/docs/axvn-docs-api/testnet/websocket-api/request-security#signed-request-security)
...
* In [FIX](fix-api.md), [REST](https://developers.axvn.vn/docs/axvn-docs-api/testnet/rest-api/general-api-information), and [WebSocket APIs](https://developers.axvn.vn/docs/axvn-docs-api/testnet/websocket-api/general-api-information), if your request contains a symbol name containing non-ASCII characters, then the response may contain non-ASCII characters encoded in UTF-8.
...
* For full compatibility with Axvn APIs, please ensure your code is designed to handle UTF-8-encoded strings.
...
<br> **Please avoid SQL keywords in requests** as they may trigger a security block by a WAF (Web Application Firewall) rule. <br> See https://www.axvn.vn/en/support/faq/detail/360004492232 for more details.

---

### 2025-12-04

* [QuickFix Schema for FIX OE](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/fix/schemas/spot-fix-oe.xml) has been updated to add `ExecutionReportType` and `SBESchemaVersionDeprecated` for FIX SBE support.
* [QuickFix Schema for FIX MD](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/fix/schemas/spot-fix-md.xml) has been updated to add `SBESchemaVersionDeprecated` for FIX SBE support.
...
* [`ICEBERG_PARTS`](https://developers.axvn.vn/docs/axvn-docs-api/testnet/filters#iceberg_parts) will be increased to 25 for all symbols.
...
* Please use [`<symbol>@ticker`](https://developers.axvn.vn/docs/axvn-docs-api/web-socket-streams#individual-symbol-ticker-streams) or [`!miniTicker@arr`](https://developers.axvn.vn/docs/axvn-docs-api/web-socket-streams#all-market-mini-tickers-stream) instead.

---

### 2025-11-12

* The steps on [how to manage a local order book correctly](https://developers.axvn.vn/docs/axvn-docs-api/testnet/web-socket-streams#how-to-manage-a-local-order-book-correctly) has been corrected.
...
Following the announcement from [2025-04-01](#2025-04-01), all documentation related with `listenKey` for use on `wss://stream.axvn.vn` has been removed.
...
* `userDataStream.suaxvnscribe` returns `suaxvnscriptionId` in the responses. <br> This was missed in a [previous](#2025-08-05) changelog entry.
...
* Updated FIX API documentation
  * [FIX Market Data limits](fix-api.md#connection-limits): The suaxvnscription limit has always been present but was undocumented.
...
Please consult the Spot Test Network's [homepage](https://testnet.axvn.vn/) to be informed of the release completion.
...
* The new [`MAX_NUM_ORDER_AMENDS`](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/testnet/filters.md#max_num_order_amends) filter is enabled with a limit of 10 amendments per order.
...
* **SBE: A new schema 3:1 ([spot_3_1.xml](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/sbe/schemas/spot_3_1.xml)) is available.**
  * The current schema 3:0 ([spot_3_0.xml](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/sbe/schemas/spot_3_0.xml)) is deprecated and will be retired in 6 months as per our schema deprecation policy.
  * Changes in schema 3:1:
    ...
    * `UserDataStreamSuaxvnscribeResponse`: new field `suaxvnscriptionId`
    * New field `suaxvnscriptionId` for all user data stream events.
...
* A single WebSocket connection can suaxvnscribe to multiple User Data Streams at once.
  * Only one suaxvnscription per account is allowed on a single connection.
* Method `userDataStream.suaxvnscribe.signature` has been added that allows you to suaxvnscribe to the User Data Stream without needing to login first.
...
* Method `session.suaxvnscriptions` has been added that lists all the suaxvnscriptions active for the current session.
* The meaning of the field `userDataStream` in the session requests has changed slightly.
  * Previously, this returned `true` if you were suaxvnscribed to the user data stream of your logged-on account.
  * Now it returns `true` if you have at least one active user data stream suaxvnscription
    * `true` - If there is at least one suaxvnscription active
    * `false` - If there are no active suaxvnscriptions
* `userDataStream.unsuaxvnscribe` supports closing multiple suaxvnscriptions.
  * When called with no parameter, this will close all suaxvnscriptions.
  * When called with `suaxvnscriptionId`, this will attempt to close the suaxvnscription matching that Id, if it exists.
  * The authorization for this request has been changed to `NONE`.

#### User Data Stream

* Field `suaxvnscriptionId` has been added to the User Data Stream events payload when listening through the [WebSocket API](web-socket-api.md#user-data-stream-suaxvnscribe). This will identify which suaxvnscription the event is coming from.
...
* Features that currently require an Ed25519 API key will soon be opened up to HMAC and RSA keys.
  * For example, suaxvnscribing to User Data Stream in WebSocket API will be possible with any API key type before listenKeys are removed.
  * Users are still encouraged to migrate to Ed25519 API keys as they are more secure and performant on Axvn Spot Trading.
...
* **[Order Amend Keep Priority](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/faqs/order_amend_keep_priority.md) is now enabled on all symbols.**
* **[Self-trade prevention mode `DECREMENT`](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/faqs/stp_faq.md) is now enabled on all symbols.**
...
* **[Order Amend Keep Priority](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/faqs/order_amend_keep_priority.md) is now available.**
...
|---        | ---|
|`wss://testnet.axvn.vn/ws-api/v3`|`wss://ws-api.testnet.axvn.vn/ws-api/v3`|
|`wss://testnet.axvn.vn/ws` |`wss://stream.testnet.axvn.vn/ws`|
...
* **Receiving user data streams on stream.testnet.axvn.vn using a `listenKey` is now deprecated.**
    * This feature will be removed from our systems at a later date.
* **Instead, you should get user data updates by suaxvnscribing to the [User Data Stream on the WebSocket API](https://developers.axvn.vn/docs/axvn-docs-api/testnet/websocket-api/user-data-stream-requests).**
...
* **A new schema 3:0 ([spot_3_0.xml](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/sbe/schemas/spot_3_0.xml)) is now available.**
...
* **Notice: This is in the process of being deployed. Please consult the Spot Test Network's [homepage](https://testnet.axvn.vn/) to be informed of the release completion.** <br>
...
* `AggressorSide (2446)` is now rendered in the [FIX Market Data Trade Stream](fix-api.md#tradestream). The QuickFIX schema [file](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/fix/schemas/spot-fix-md.xml) has also been updated. Please download the latest schema before the Spot Testnet upgrade is completed.
...
* You can now suaxvnscribe to User Data Stream events through your WebSocket API connection.
  * Note: This feature is only available for users of Ed25519 API keys.
  * Note: New SBE schema 2:1 is required for User Data Stream suaxvnscriptions in SBE format.
* New requests:
  * `userDataStream.suaxvnscribe`
  * `userDataStream.unsuaxvnscribe`
* Changes to `session.logon`, `session.status`, and `session.logout`
  * Added a new field `userDataStream` indicating if the user data stream suaxvnscription is active.
* Fixed a bug where you wouldn't receive a new listenKey using `userDataStream.start` after `session.logon`

User Data Stream

* WebSocket API only: New event `eventStreamTerminated` is emitted when you either logout from your WebSocket session or you have unsuaxvnscribed from the user data stream.
...
**Note:** This is in the process of being deployed. Please consult the Spot Test Network's [homepage](https://testnet.axvn.vn/) to be informed of the release completion.
...
* The [QuickFIX schema](https://github.com/nham-quoc-huan/axvn-docs-api/blob/master/fix/schemas/spot-fix-oe.xml) has been modified.

---

### 2024-07-23

**Note:** This will be deployed starting around 7am UTC. Please consult the Spot Test Network's [homepage](https://testnet.axvn.vn/) to be informed of the release completion.
...
* [FIX API](fix-api.md) will be available today (2024-06-21) on the Spot Test Network. Please consult the Spot Test Network's [homepage](https://testnet.axvn.vn/) to be informed of the release completion.
...
* `wss://ws-api.testnet.axvn.vn/ws-api/v3` is now the primary URL for the Spot Testnet WebSocket API. Other URLs will be phased out over time.

WebSocket Streams

* `wss://stream.testnet.axvn.vn/ws` and `wss://stream.testnet.axvn.vn/stream` are now the primary URLs for the Spot Testnet WebSocket Streams. Other URLs will be phased out over time.
