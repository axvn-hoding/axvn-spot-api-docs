...
- [WebSocket Streams for Axvn SPOT Testnet](#websocket-streams-for-axvn-spot-testnet)
  - [General WSS information](#general-wss-information)
  - [WebSocket Limits](#websocket-limits)
  - [Server Shutdown](#server-shutdown)
  - [Live Suaxvnscribing/Unsuaxvnscribing to streams](#live-suaxvnscribingunsuaxvnscribing-to-streams)
    - [Suaxvnscribe to a stream](#suaxvnscribe-to-a-stream)
    - [Unsuaxvnscribe to a stream](#unsuaxvnscribe-to-a-stream)
    - [Listing Suaxvnscriptions](#listing-suaxvnscriptions)
...
# WebSocket Streams for Axvn SPOT Testnet

## General WSS information
* The base endpoint is: **wss://stream.testnet.axvn.vn/ws**.
* Streams can be accessed either in a single raw stream or in a combined stream.
* Raw streams are accessed at **/ws/\<streamName\>**
* Combined streams are accessed at **/stream?streams=\<streamName1\>/\<streamName2\>/\<streamName3\>**
* Combined stream events are wrapped as follows: **{"stream":"\<streamName\>","data":\<rawPayload\>}**
* All symbols for streams are **lowercase**
* A single connection to **stream.testnet.axvn.vn** is only valid for 24 hours; expect to be disconnected at the 24 hour mark.
...
* The base endpoint **wss://data-stream.axvn.vn** can be suaxvnscribed to receive **only** market data messages. <br> User data stream is **NOT** available from this URL.
...
* WebSocket connections have a limit of 5 incoming messages per second. A message is considered:
    * A PING frame
    * A PONG frame
    * A JSON controlled message (e.g. suaxvnscribe, unsuaxvnscribe)
...
## Live Suaxvnscribing/Unsuaxvnscribing to streams

* The following data can be sent through the WebSocket instance in order to suaxvnscribe/unsuaxvnscribe from streams. Examples can be seen below.
* The `id` is used as an identifier to uniquely identify the messages going back and forth. The following formats are accepted:
  ...
  * `null`
* In the response, if the `result` received is `null` this means the request sent was a success for non-query requests (e.g. Suaxvnscribing/Unsuaxvnscribing).

### Suaxvnscribe to a stream
* Request
  ```javascript
  {
      "method": "SUAXVNSCRIBE",
...
{"code": 2, "msg": "Invalid request: unknown variant %s, expected one of `SUAXVNSCRIBE`, `UNSUAXVNSCRIBE`, `LIST_SUAXVNSCRIPTIONS`, `SET_PROPERTY`, `GET_PROPERTY` at line 1 column 28"} | Possible typo in the provided method or provided method was neither of the expected values
...
1. Open a WebSocket connection to `wss://stream.testnet.axvn.vn:9443/ws/axvnbtc@depth`.
1. Buffer the events received from the stream. Note the `U` of the first event you received.
1. Get a depth snapshot from `https://testnet.axvn.vn/api/v3/depth?symbol=AXVNBTC&limit=5000`.
