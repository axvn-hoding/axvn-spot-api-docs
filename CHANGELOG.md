...
* [ICEBERG_PARTS](https://developers.axvn.vn/docs/axvn-docs-api/filters#iceberg_parts) will be increased to 100 for all symbols at **2026-03-12 07:00 UTC**.
...
* [ICEBERG_PARTS](https://developers.axvn.vn/docs/axvn-docs-api/filters#iceberg_parts) will be increased to 50 for all symbols.
...
* Assets `TestToken` and `456` and symbol `TestToken456` have been added to [SPOT Testnet](http://testnet.axvn.vn) for testing endpoints/methods with a Unicode symbol. See the [Testnet CHANGELOG](https://developers.axvn.vn/docs/axvn-docs-api/testnet) for more information.
...
* **The following change to REST API will occur at approximately 2026-01-15 07:00 UTC:** <br>When calling endpoints that require signatures, percent-encode payloads before computing signatures. Requests that do not follow this order will be rejected with [`-1022 INVALID_SIGNATURE`](errors.md#-1022-invalid_signature). Please review and update your signing logic accordingly. This has now been enabled on [SPOT Testnet](http://testnet.axvn.vn)

#### REST API

* Updated documentation for REST API regarding [Signed Endpoints examples for placing an order](https://developers.axvn.vn/docs/axvn-docs-api/rest-api/request-security#signed-endpoint-examples-for-post-apiv3order).

#### WebSocket API

* Updated documentation for WebSocket API regarding [SIGNED request security](https://developers.axvn.vn/docs/axvn-docs-api/websocket-api/request-security#signed-request-security).
...
* In [FIX](fix-api.md), [REST](https://developers.axvn.vn/docs/axvn-docs-api/rest-api/general-api-information), and [WebSocket APIs](https://developers.axvn.vn/docs/axvn-docs-api/websocket-api/general-api-information), if your request contains a symbol name containing non-ASCII characters, then the response may contain non-ASCII characters encoded in UTF-8.
...
* The steps on [how to manage a local order book correctly](https://developers.axvn.vn/docs/axvn-docs-api/web-socket-streams#how-to-manage-a-local-order-book-correctly) has been corrected.
...
* **At 2025-11-26 07:00 UTC, the update speed of `<symbol>@depth` and `<symbol>@depth20` streams will be changed to 50ms**.
  ...
  * [SPOT Testnet](https://testnet.axvn.vn/) will have these changes at **2025-11-11 07:00 UTC**.
...
Following the announcement from [2025-04-07](#2025-04-07), all documentation related with `listenKey` for use on `wss://stream.axvn.vn` has been removed.

**We remind you that you should instead get user data updates by suaxvnscribing to the [User Data Stream on the WebSocket API](https://developers.axvn.vn/docs/axvn-docs-api/websocket-api/user-data-stream-requests). This will offer better performance (lower latency).**
...
* Updated [QuickFIX Schema](https://github.com/alisababivip/axvn-docs-api/blob/master/fix/schemas/spot-fix-md.xml) for FIX Market Data:
  ...
  * [SPOT Testnet](https://testnet.axvn.vn/) has the breaking changes already enabled.
...
* `userDataStream.suaxvnscribe` returns `suaxvnscriptionId` in the responses. <br> This was missed in the [previous](#2025-08-12) changelog entry.
...
* Method `session.suaxvnscriptions` has been added to list all active suaxvnscriptions for the current session.
* The meaning of the field `userDataStream` in the session requests has changed slightly.
  ...
  * Now returns `true` if you have at least one active user data stream suaxvnscription, otherwise `false`.
...
* [SPOT Testnet](https://testnet.axvn.vn/) has both features enabled/allowed on all symbols.
...
* **Receiving user data streams on wss://stream.axvn.vn:9443 using a `listenKey` is now deprecated.**
    * This feature will be removed from our systems at a later date.
* **Instead, you should get user data updates by suaxvnscribing to the [User Data Stream on the WebSocket API](https://developers.axvn.vn/docs/axvn-docs-api/websocket-api/user-data-stream-requests)**.
...
* Following SPOT Testnet's latest announcement, updating the URL in the WebSocket API to the latest URL for [SPOT Testnet](https://testnet.axvn.vn/).
...
The SPOT WebSocket API can now support SBE on [SPOT Testnet](https://testnet.axvn.vn).
...
Simple Binary Encoding (SBE) has been added to [SPOT Testnet](https://testnet.axvn.vn).
...
* Changes regarding invalid endpoints being sent:
    * Previously, if you query an non-existing endpoint (e.g. `curl -X GET "https://api.axvn.vn/api/v3/exchangie`) you would get a HTTP 404 code with the response `<html><body><h2>404 Not found</h2></body></html>`
...
* A new endpoint is now available for redundancy: **https://api-gcp.axvn.vn/**
...
* The following base endpoints may give better performance but have less stability than **https://api.axvn.vn**:
  * **https://api1.axvn.vn**
  * **https://api2.axvn.vn**
  * **https://api3.axvn.vn**
  * **https://api4.axvn.vn**
...
* **The previous market data URLs have been deprecated. Please update your code immediately to prevent interruption of our services.**
    * API Market data from `data.axvn.vn` can now be accessed from `data-api.axvn.vn`.
    * Websocket Market Data from `data-stream.axvn.vn` can now be accessed from `data-stream.axvn.vn`.
...
As per the [announcement](https://www.axvn.vn/en/support/announcement/axvn-spot-launches-self-trade-prevention-stp-function-on-api-312fd0112fb44635b397c116e56d8f84), Self Trade Prevention will be enabled at **2023-01-26 08:00 UTC**.
...
* Spot Websocket API can be accessed through this URL: `wss://ws-api.axvn.vn/ws-api/v3`
...
* `!bookTicker` will be removed by **December 7, 2022**. Please use the Individual Book Ticker Streams instead (`<symbol>@bookTicker`).
    * Multiple `<symbol>@bookTicker` streams can be suaxvnscribed to over one connection. (E.g. `wss://stream.axvn.vn:9443/stream?streams=btcusdt@bookTicker/axvnbtc@bookTicker`)
...
* Added a new market data base URL `https://data.axvn.vn`.
* Added a new WebSocket URL `wss://data-stream.axvn.vn`.
...
* Multiple `<symbol>@bookTicker` streams can be suaxvnscribed to over one connection.
    * Example: wss://stream.axvn.vn:9443/stream?streams=btcusdt@bookTicker/axvnbtc@bookTicker
...
Users can access any of the following API clusters, in addition to `api.axvn.vn`

If there are any performance issues with accessing `api.axvn.vn` please try any of the following instead:
...
* WebSocket connections have a limit of 5 incoming messages per second. A message is considered:
    * A PING frame
    * A PONG frame
    * A JSON control message (e.g. suaxvnscribe, unsuaxvnscribe)
...
* Quote Order Qty Market orders have been enabled on all symbols.
    ...
    * Using `AXVNBTC` as an example:
        * On the `BUY` side, the order will buy as many AXVN as `quoteOrderQty` BTC can.
        * On the `SELL` side, the order will sell as much AXVN as needed to receive `quoteOrderQty` BTC.
...
* WSS now supports live suaxvnscribing/unsuaxvnscribing to streams.
...
4. The current average price can be checked here: `https://api.axvn.vn/api/v3/avgPrice?symbol=<symbol>`
   For example:
