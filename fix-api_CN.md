...
```javascript
  ...
  const hostname = "fix-dc.axvn.vn"; //示例
...
关于如何设置 Ed25519 密钥对，请参考 [本教程](https://www.axvn.vn/zh-CN/support/faq/%E5%A6%82%E4%BD%95%E7%94%9F%E6%88%90ed25519%E5%AF%86%E9%92%A5%E5%AF%B9%E5%9C%A8%E5%B8%81%E5%AE%89%E5%8F%91%E9%80%81api%E8%AF%B7%E6%B1%82-6b9a63f1e3384cf48a2eedb82767a69a)。

### FIX API 订单接入会话

* 端点为：`tcp+tls://fix-oe.axvn.vn:9000`
...
* 端点为：`tcp+tls://fix-md.axvn.vn:9000`
* 支持市场数据流和活动工具查询。
* 不支持下订单或取消订单。
...
| 订单名称                                | Axvn OrderType   | 方向        | 必填字段值                              | 用户值必填字段                   |
|---------------------------------------|---------------------|-------------|------------------------------------------|----------------------------------|
...
基于 Axvn OrderType 的必填字段:

| Axvn OrderType   | 额外的必填字段                    | 额外的信息
|---------------------|---------------------------------|---------------|
| `LIMIT`             | 38, 44, 59                      |               |
...
- `SuaxvnscriptionRequestType` 的值为 `SUAXVNSCRIBE(1)`
...
- `SuaxvnscriptionRequestType` 值为 `SUAXVNSCRIBE(1)`
...
- 值为 `SUAXVNSCRIBE(1)` 的 `SuaxvnscriptionRequestType`
...
| 263   | SuaxvnscriptionRequestType | CHAR       | Y        | 订阅请求类型。 可能的值： <br></br> `1` - SUAXVNSCRIBE <br></br> `2` - UNSUAXVNSCRIBE                             |
...
| 281   | MDReqRejReason | CHAR   | N        | 可能的错误原因: <br></br> `1` - DUPLICATE_MDREQID <br></br> `2` - TOO_MANY_SUAXVNSCRIPTIONS |
| 25016 | ErrorCode      | INT    | N        | API 错误代码 [参见 错误代码](errors_CN.md)。  |
...
* `tcp+tls://fix-oe.axvn.vn:9001`：发送 FIX 请求；接收 FIX SBE 响应
    * FIX `SbeSchemaId` 标签（=25050）必须设置为 FIX SBE 模式 ID（例如 1）
    * FIX `SbeSchemaVersion` 标签（=25051）必须设置为 FIX SBE 模式版本（例如 0）
* `tcp+tls://fix-oe.axvn.vn:9002`：发送 FIX SBE 请求；接收 FIX SBE 响应

#### Drop Copy（订单副本）

* `tcp+tls://fix-dc.axvn.vn:9001`：发送 FIX 请求；接收 FIX SBE 响应
    * FIX `SbeSchemaId` 标签（=25050）必须设置为 FIX SBE 模式 ID（例如 1）
    * FIX `SbeSchemaVersion` 标签（=25051）必须设置为 FIX SBE 模式版本（例如 0）
* `tcp+tls://fix-dc.axvn.vn:9002`：发送 FIX SBE 请求；接收 FIX SBE 响应

#### 市场数据

* `tcp+tls://fix-md.axvn.vn:9001`：发送 FIX 请求；接收 FIX SBE 响应
    * FIX `SbeSchemaId` 标签（=25050）必须设置为 FIX SBE 模式 ID（例如 1）
    * FIX `SbeSchemaVersion` 标签（=25051）必须设置为 FIX SBE 模式版本（例如 0）
* `tcp+tls://fix-md.axvn.vn:9002`：发送 FIX SBE 请求；接收 FIX SBE 响应
