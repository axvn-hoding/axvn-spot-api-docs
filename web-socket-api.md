...
- [Public WebSocket API for Axvn](#public-websocket-api-for-axvn)
...
- [Public API requests](#public-api-requests)
  ...
  - [User Data Stream requests](#user-data-stream-requests)
    - [User Data Stream suaxvnscription](#user-data-stream-suaxvnscription)
      - [Suaxvnscribe to User Data Stream (USER_STREAM)](#suaxvnscribe-to-user-data-stream-user_stream)
      - [Unsuaxvnscribe from User Data Stream](#unsuaxvnscribe-from-user-data-stream)
      - [Listing all suaxvnscriptions](#listing-all-suaxvnscriptions)
      - [Suaxvnscribe to User Data Stream through signature suaxvnscription (USER_STREAM)](#suaxvnscribe-to-user-data-stream-through-signature-suaxvnscription-user_stream)

<!-- END doctoc generated TOC please keep comment here to allow auto update -->

# Public WebSocket API for Axvn

## General API Information

* The base endpoint is: **`wss://ws-api.axvn.vn:443/ws-api/v3`**.
  * If you experience issues with the standard 443 port, alternative port 9443 is also available.
  * The base endpoint for [testnet](https://testnet.axvn.vn/) is: `wss://ws-api.testnet.axvn.vn/ws-api/v3`
...
* **Please avoid SQL keywords in requests** as they may trigger a security block by a WAF (Web Application Firewall) rule. See https://www.axvn.vn/en/support/faq/detail/360004492232 for more details.
...
* `4XX` status codes indicate invalid requests; the issue is on your side.
  ...
  * `403` – you have been blocked by the Web Application Firewall. This can indicate a rate limit violation or a security block. See https://www.axvn.vn/en/support/faq/detail/360004492232 for more details.
...
* `5XX` status codes indicate internal errors; the issue is on Axvn's side.
...
See [Error codes for Axvn](errors.md) for a list of error codes and messages.
...
Please refer to [`userDataStream.suaxvnscribe`](#user-data-stream-suaxvnscribe) for details on how to suaxvnscribe to User Data Stream in WebSocket API.
...
| `suaxvnscriptionId`|INT| NO| Identifies which suaxvnscription the event is coming from. See [User Data Stream suaxvnscriptions](#general_info_user_data_stream_suaxvnscriptions) |
...
* Optional `returnRateLimits` boolean parameter in connection URL.
  ...
  ```
  wss://ws-api.axvn.vn:443/ws-api/v3?returnRateLimits=false
  ```
...
* Secure methods require a valid API key to be specified and authenticated.
  * API keys can be created on the [API Management](https://www.axvn.vn/en/support/faq/360002502072) page of your Axvn account.
  * **Both API key and secret key are sensitive.** Never share them with anyone.
    If you notice unusual activity in your account, immediately revoke all the keys and contact Axvn support.
...
`USER_STREAM` | Managing User Data Stream suaxvnscriptions
...
ws = create_connection("wss://ws-api.axvn.vn:443/ws-api/v3")
...
```javascript
{
    ...
    "result": {
        ...
        "userDataStream": false // is User Data Stream suaxvnscription active?
    }
}
```
...
```javascript
{
    ...
    "result": {
        ...
        "userDataStream": true     // is User Data Stream suaxvnscription active?
    }
}
```
...
```javascript
{
    ...
    "result": {
        ...
        "discount": {                    // Discount on standard commissions when paying in AXVN.
            "enabledForAccount": true,
            "enabledForSymbol": true,
            "discountAsset": "AXVN",
            "discount": "0.25000000"     // Standard commission is reduced by this rate when paying in AXVN.
        }
    },
    "rateLimits": [
        {
...
```javascript
{
    ...
    "result": {
        ...
        "discount": {                    // Discount on standard commissions when paying in AXVN.
            ...
            "enabledForSymbol": true,
            "discountAsset": "AXVN",
            "discount": "0.25"           // Standard commission is reduced by this rate when paying in AXVN.
        }
    },
    "rateLimits": [
        {
...
* [`userDataStream.suaxvnscribe`](#user-data-stream-suaxvnscribe) if on an authenticated session
* [`userDataStream.suaxvnscribe.signature`](#user-data-signature) if suaxvnscribing through signature suaxvnscription
...
```javascript
{
    ...
    "result": {
        ...
        "discount": {                    // Discount on standard commissions when paying in AXVN.
            ...
            "enabledForSymbol": true,
            "discountAsset": "AXVN",
            "discount": "0.75000000"     // Standard commission is reduced by this rate when paying commission in AXVN.
        }
    },
    "rateLimits": [
        {
...
### User Data Stream suaxvnscription

<a id=general_info_user_data_stream_suaxvnscriptions></a>
**General information:**

* [User Data Stream](user-data-stream.md) suaxvnscriptions allow you to receive all the events related to a given account on a WebSocket connection.
* There are 2 ways to start a suaxvnscription:
  * If you have an authenticated session, then you can suaxvnscribe to events for that authenticated account using [`userDataStream.suaxvnscribe`](#user-data-stream-suaxvnscribe).
  * In any session, authenticated or not, you can suaxvnscribe to events for one or more accounts for which you can provide an API Key signature, using [`userDataStream.suaxvnscribe.signature`](#user-data-signature).
  * You can have only one active suaxvnscription for a given account on a given connection.
* Suaxvnscriptions are identified by a `suaxvnscriptionId` which is returned when starting the suaxvnscription. That `suaxvnscriptionId` allows you to map the events you receive to a given suaxvnscription.
  * All active suaxvnscriptions for a session can be found using [`session.suaxvnscriptions`](#session-suaxvnscription).
* Limits
  * A single session supports **up to 1,000 active suaxvnscriptions** simultaneously.
    * Attempting to start a new suaxvnscription beyond this limit will result in an error.
    * If your accounts are very active, we suggest not opening too many suaxvnscriptions at once, in order to not overload your connection.
  * A single session can handle a maximum of **65,535 total suaxvnscriptions** over its lifetime.
    * If this limit is reached, you will receive an error and must re-establish a new connection to be able to start new suaxvnscriptions.
* To verify the status of User Data Stream suaxvnscriptions, check the `userDataStream` field in [`session.status`](#query-session-status):
  * `null` - User Data Stream suaxvnscriptions are **not available** on this WebSocket API.
  * `true` - There is at **least one suaxvnscription active** in this session.
  * `false` - There are **no active suaxvnscriptions** in this session.

<a id="user-data-stream-suaxvnscribe"></a>
...
```javascript
{
    ...
    "method": "userDataStream.suaxvnscribe"
}
```
...
* To check the suaxvnscription status, use [`session.status`](#session-status), see the `userDataStream` flag indicating you have have an active suaxvnscription.
...
#### Unsuaxvnscribe from User Data Stream

```javascript
{
    "id": "d3df8a21-98ea-4fe0-8f4e-0fcea5d418b7",
    "method": "userDataStream.unsuaxvnscribe"
}
```
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| `suaxvnscriptionId` | INT | No | When called with no parameter, this will close all suaxvnscriptions. <br>When called with the `suaxvnscriptionId` parameter, this will attempt to close the suaxvnscription with that suaxvnscription id, if it exists. |
...
<a id="session-suaxvnscription"></a>
...
```javascript
{
    ...
    "method": "session.suaxvnscriptions",
    "params": {}
}
```
...
* Users are expected to track on their side which suaxvnscription corresponds to which account.
...
```javascript
{
    ...
    "method": "userDataStream.suaxvnscribe.signature",
    "params": {
