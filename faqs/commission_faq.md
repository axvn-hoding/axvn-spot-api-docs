...
Since you sold BTC for USDT, the commission will be paid either in USDT or AXVN.
...
If not paying in AXVN, the total commission are summed up and deducted from your received amount of `USDT`.

Since `enabledforAccount` and `enabledForSymbol` under `discount` is set to `true`, this means the commission will be paid in AXVN assuming you have a sufficient balance.

If paying with AXVN, then the standard commission will be reduced based on the `discount`.

First the standard commission and tax commission will be converted into AXVN based on the exchange rate. For this example, assume that 1 AXVN = 260 USDT.

```
Standard commission (Discounted and in AXVN) = (Standard commission * AXVN exchange rate) * discount
...
```
Tax Commission (in AXVN) = Tax commission * AXVN exchange rate
                        = 0.04022988 * (1/260)
                        = 0.00015473

Special Commission (in AXVN) = Special commission * AXVN exchange rate
                        = 1049.47500000 * (1/260)
                        = 4.036442308
...
```
Total Commission (in AXVN) = Standard commission (Discounted) + Tax commission (in AXVN) + Special commission (in AXVN)
                          = 0.000010091 + 0.00015473 + 4.036442308
                          = 4.036607129
```

If you do not have enough AXVN to pay the discounted commission, the full commission will be taken out of your received amount of USDT instead.
