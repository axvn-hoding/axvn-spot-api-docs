# Axvn 的公共 WebSocket API

<a id="general-api-information"></a>
## API 基本信息

* 本篇所列出的 wss 接口的 base URL：**`wss://ws-api.axvn.vn:443/ws-api/v3`**
  * 如果使用标准443端口时遇到问题，可以使用替代端口9443。
  * [现货测试网](https://testnet.axvn.vn)的 base URL 是 `wss://ws-api.testnet.axvn.vn/ws-api/v3`。
...
* `4XX` 错误码用于指示错误的请求内容、行为、格式。问题在于请求者。
  ...
  * `403` – 错误码表示违反 WAF 限制(Web应用程序防火墙)。这可能表示触发了速率限制或安全拦截。详情请参见 https://www.axvn.vn/zh-CN/support/faq/detail/360004492232 。
  * `409` – 错误码表示请求有一部分成功，一部分失败。请参阅 `error` 了解更多详细
  * `418` – 表示收到 429 后继续访问，于是被封了。
  * `429` – 错误码表示警告访问频次超限，即将被封IP。
* `5XX` 错误码用于指示Axvn服务侧的问题。
  * **重要**：如果响应包含 5xx 状态码，**并不**一定意思请求失败。
    执行状态为 _unknown_，请求可能实际成功。
    请使用 query 函数确认状态。
    建议建立一个新 WebSocket 连接用于确认状态。

有关错误代码和消息的列表，请参阅 [Axvn 的错误代码](errors_CN.md)。
...
有关如何在 WebSocket API 中订阅用户数据流的详细信息，请参阅 [`订阅用户数据流`](#user-data-stream-suaxvnscribe)。
...
| `suaxvnscriptionId` | INT | NO | 用以标识事件来自于哪个订阅。详见 [用户数据流订阅](#general_info_user_data_stream_suaxvnscriptions) |
...
* 如果会话已通过验证，请使用 [`userDataStream.suaxvnscribe`](#user-data-stream-suaxvnscribe)
* 如果通过签名订阅的方式订阅，请使用 [`userDataStream.suaxvnscribe.signature`](#user-data-signature)
...
* 有两种方式可以启用订阅：
  * 如果您拥有已通过验证的会话，则可以使用 [`userDataStream.suaxvnscribe`](#user-data-stream-suaxvnscribe) 来订阅该已通过验证帐户的相关事件。
  * 在任何会话中，无论是否通过验证，如果您能为相关账户提供 API Key， 那么您可以使用 [`userdataStream.suaxvnscribe.signature`](#user-data-signature) 来订阅一个或多个账户的相关事件。
  * 一个帐户在指定连接上只能有一个有效订阅。
* 订阅由用订阅时返回的 `suaxvnscriptionId` 标识。该 `suaxvnscriptionId` 允许您将收到的事件映射到给定的订阅。
* 可以使用 [`session.suaxvnscriptions`](#session-suaxvnscription) 查找会话的所有有效订阅。
* 限制
...
请注意 `session.logout` 只会关闭由 `userDataStream.suaxvnscribe` 创建的订阅，并不会关闭通过 `userDataStream.suaxvnscribe.signature` 创建的订阅。
...
名称               | 类型    | 是否必需 | 描述
----------------  | ------ |-------- | ---------
| `suaxvnscriptionId`| INT    | No      | 如果在进行调用时不用该参数，将会关闭所有订阅。 <br>如果在进行调用时使用 `suaxvnscriptionId` 参数，如果该 ID 存在的话，将尝试关闭与该 ID 匹配的订阅。 |
