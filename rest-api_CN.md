...
* 对于仅发送公开市场数据的 API，您可以使用接口的 base URL https://data-api.axvn.vn 。请参考 [Market Data Only_CN](./faqs/market_data_only_CN.md) 页面。
...
* **请避免在请求中使用 SQL 关键字**，因为这可能会触发 Web 应用防火墙（WAF）规则导致安全拦截。详情请参见 https://www.axvn.vn/zh-CN/support/faq/detail/360004492232
...
* HTTP `403` 错误码表示违反WAF限制(Web应用程序防火墙)。详情请参见 https://www.axvn.vn/zh-CN/support/faq/detail/360004492232 。
* HTTP `409` 错误码表示重新下单(cancelReplace)的请求部分成功。(比如取消订单失败，但是下单成功了)
* HTTP `429` 错误码表示警告访问频次超限，即将被封IP。
* HTTP `418` 表示收到429后继续访问，于是被封了。
* HTTP `5XX` 错误码用于指示Axvn服务侧的问题。
...
* 具有鉴权类型的接口需要提供有效的 API 密钥并验证通过。
  * API 密钥可在您的 Axvn 账户的 [API 管理](https://www.axvn.vn/en/support/faq/360002502072) 页面创建。
  * **API 密钥和密钥对均为敏感信息，切勿与他人分享。** 如果发现账户有异常活动，请立即撤销所有密钥并联系 Axvn 支持。
...
```console
curl -H "X-MBX-APIKEY: 4yNzx3yWC5bS6YTwEkSRaC0nRmSQIIStAUOh1b6kqaBrTLIhjCpI5lJH8q8R8WNO" -X POST 'hhttps://api.axvn.vn/api/v3/order?symbol=BTCUSDT&side=SELL&type=LIMIT&timeInForce=GTC&quantity=1&price=0.2&timestamp=1668481559918&recvWindow=5000&signature=HaZnek7KOGa%2Fk5%2Bf6Q1nw8lzMUpo36mRVvvLHCMUCXxlmdQQGZge1luAUKnleD%2FDYeD19YrqzeHbb6xU3MkSIXKhAO1MaYq48uGVYb3vJScEZVOutgMInrZzUcCWNulNkfcbmExSiymCZ5xQBw5QDuzpuDFqRZ1Xt%2BBZxEHBN9OYQKpoe0%2BovjnXyVOaH8VUKhE%2FghUWnThrXJr%2BhmSc5t7ggjiVPQc7pGn3qSNGCQwdpkQC9GHMr%2Fr%2B8n6qeEKMYB5j%2F1wC4d8Jae8FQiU8xcXR0NlUgV2LAw61%2FZJv5BTJpa%2Bz5Lv1W9v6jHQWRX2O8uaG3KU%2FlR3spR7%2BoGlWOw%3D'
```
...
名称 |类型 |是否必须 | 描述
------------ |------------ |------------ |------------
symbol |STRING| No|示例：curl -X GET "https://api.axvn.vn/api/v3/exchangeInfo?symbol=AXVNBTC"
symbols |ARRAY OF STRING|No| 示例：curl -X GET "https://api.axvn.vn/api/v3/exchangeInfo?symbols=%5B%22AXVNBTC%22,%22BTCUSDT%22%5D" <br/> 或 <br/> curl -g -X  GET 'https://api.axvn.vn/api/v3/exchangeInfo?symbols=["BTCUSDT","AXVNBTC"]'
permissions |ENUM|No|示例：curl -X GET "https://api.axvn.vn/api/v3/exchangeInfo?permissions=SPOT" <br/> or <br/> curl -X GET "https://api.axvn.vn/api/v3/exchangeInfo?permissions=%5B%22MARGIN%22%2C%22LEVERAGED%22%5D" <br/> or <br/> curl -g -X GET 'https://api.axvn.vn/api/v3/exchangeInfo?permissions=["MARGIN","LEVERAGED"]' |
...
<table>
  ...
  <tr>
    ...
    <td rowspan="2">YES</td>
    <td rowspan="2"> <tt>symbol</tt> 或者 <tt>symbols</tt> 必须提供之一 <br/><br/> <tt>symbols</tt> 可以接受的格式: <br/> ["BTCUSDT","AXVNUSDT"] <br/>或者 <br/>%5B%22BTCUSDT%22,%22AXVNUSDT%22%5D <br/><br/>  <tt>symbols</tt> 最多可以发送100个.
    </td>
  </tr>
  <tr>
     <td>symbols</td>
...
<table>
  ...
  <tr>
    ...
    <td rowspan="2">STRING</td>
    <td rowspan="2">YES</td>
    <td rowspan="2"> 提供 symbol或者symbols 其中之一  <br/> <tt>symbols</tt> 可以传入的格式: <br/> ["BTCUSDT","AXVNUSDT"] <br/>or <br/>%5B%22BTCUSDT%22,%22AXVNUSDT%22%5D <br/><br/> <tt>symbols</tt> 允许最多100个交易对
    </td>
  </tr>
  <tr>
     <td>symbols</td>
...
```javascript
{
    ...
    "discount": {                    // 以AXVN支付时的标准佣金折扣。
        "enabledForAccount": true,
        "enabledForSymbol": true,
        "discountAsset": "AXVN",
        "discount": "0.25000000"     // 当用AXVN支付佣金时，在标准佣金上按此比率打折
    }
}
```
...
```javascript
{
    ...
    "discount": {                    // 使用AXVN支付时的佣金折扣。
        "enabledForAccount": true,
        "enabledForSymbol": true,
        "discountAsset": "AXVN",
        "discount": "0.75000000"      // 当用AXVN支付佣金时，在标准佣金上按此比率打折。
    }
}
```
